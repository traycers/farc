...
Ровно поэтому DFS-порядок принят как порядок хранения всего TOC целиком (`06-toc-format.md` §5–§6), а не только его части — TOC строится при финализации фконтейнера, когда дерево уже неизменно, и переупорядочивание не требует трогать «Данные» (§8.4 ниже).
...
- **Дерево «Данных»** (`05-data-format.md` §3) — это в точности пара `parent` + `sibling` из §3.6, хранимая как **массив структур (AoS)**, а не как параллельные векторы (SoA). Причина — конкурентная запись: узел добавляется одной операцией, а не согласованной записью в несколько массивов сразу (`05-data-format.md` §2). Вся теория этой главы применима без изменений: SoA/AoS — вопрос раскладки в памяти, а не структуры.
...
- **TOC** (`06-toc-format.md`), в отличие от «Данных», строится позже — одним процессом на финализации, когда дерево уже неизменно, то есть ровно в условиях Hsu (§3.7). Именно поэтому он хранится как SoA, а не AoS: тот же материал (`type`/`role`/`parent`/`sibling`), другая раскладка, плюс DFS-переупорядочение (§8) с хранением обеих таблиц соответствия (§7.2), чтобы `parent`/`sibling` оставались разрешимыми после перестановки (`06-toc-format.md` §5). В farc `sibling` в TOC хранится безусловно, даже несмотря на общее замечание §8.4 о его избыточности после DFS — он остаётся единственным документированным способом найти ключевой кадр GOP (`07-media-tree.md` §6), а отбрасывать его в пользу арифметики по позиции и предполагаемому размеру поддерева означало бы зашивать в код доменное предположение о постоянной ширине поддерева `frame`, которое переживёт не любое будущее изменение состава его детей.
