...
| Ближайший общий предок `a`, `b` | длина общего префикса строк |
...
Пропуск внешнего `old2new` — самая частая ошибка при переупорядочении: дерево остаётся структурно валидным по форме (все значения в диапазоне), но описывает совершенно другое дерево. Отсюда практическое требование: **после переупорядочения обязательна проверка инвариантов** (§11), она ловит этот класс ошибок мгновенно.
...
## 10. Сводка: три порядка и их свойства
...
- **Инвариант `parent ≤ id`, `sibling ≤ id`** (`05-data-format.md` §3) даёт однопроходные propagation (§5.3), бесплатную ацикличность (§11) и восстановление дерева последовательным сканированием «Данных» при потере TOC.
