...
Справочная глава: как устроены деревья на базе массивов в подходе Aaron W. Hsu (co-dfns, APL) и как с ними работать. Первоисточники со ссылками — §17.1, видеодоклады — §17.3. Документ объясняет теорию, на которую опирается формат дерева фконтейнера (`05-data-format.md` §2–§3) и формат TOC (`06-toc-format.md` §5–§6).
...
Позиция узла в массиве при этом структурного смысла не несёт: то же самое дерево можно записать в любом другом порядке, в том числе с корнем не в нулевой ячейке (§7.4). Порядок создания — соглашение farc, а не требование представления.
...
Обе таблицы стоит сохранять, а не пересчитывать: `new2old` нужна, чтобы отдать наружу исходный id узла (например, ссылку в «Контент» фконтейнера), `old2new` — чтобы принять снаружи старый id и найти его новую позицию.
...
Демонстрация: перемешаем пример из §2 перестановкой, не связанной ни с DFS, ни с BFS, ни с порядком создания:
...
Корень (старый id 0) оказался на позиции 2, `channel(1)` — на позиции 5, кадры разбросаны. Множество рёбер при этом не изменилось: это по-прежнему в точности дерево из §2, и все алгоритмы §5, §6, §10 дают на нём те же ответы. Инвариант порядка нарушен почти везде — `parent′[i] <= i` не выполняется у большинства узлов.

Теряется ровно то, что перечислено в §5.3, — однопроходность. Маска поддерева `channel(1)`, эталон `{2, 4, 6, 8, 9, 10, 11}` (номера старые):
...
Методы, работающие «до неподвижной точки», к порядку безразличны — они вообще не смотрят на нумерацию. Однопроходные версии на порядок опираются, и точное условие для них — не «порядок создания», а **топологическая упорядоченность массива в одну или другую сторону**. Это тонкость: например, если взять дерево из §2 и просто обратить порядок узлов, то `parent[i] >= i`, и корректным станет один проход **назад**, а не вперёд. farc получает топологическую упорядоченность даром — как побочный эффект того, что узел физически создаётся после своего родителя (`05-data-format.md` §3).
...
Ровно поэтому DFS-порядок принят как порядок хранения всего TOC целиком (`06-toc-format.md` §5–§6), а не только его части — TOC строится при финализации фконтейнера, когда дерево уже неизменно, и переупорядочивание не требует трогать «Контент» (§8.4 ниже).
...
Зачем это в farc: «в каком общем узле лежат все найденные кадры» — минимальное поддерево, покрывающее результат поиска по TOC (`06-toc-format.md` §7). Ответ на такой вопрос показывает, в пределах одного ли канала/потока/версии параметров лежит выборка, и позволяет читать один диапазон вместо разрозненных узлов.
...
## 12. Сводка: четыре порядка и их свойства

Все четыре описывают одно и то же дерево (§7.4) — различаются только тем, что каждый из них даёт даром:
...
| Однопроходный propagation (§5.3) | **нет** | да | да | да |
...
| Естественная фаза в farc | — | запись фконтейнера | финализация, TOC | послойные обходы |

Первый столбец — не гипотетический: это то, что представление допускает само по себе, без дополнительных соглашений. Остальные три — соглашения, которые farc и Hsu накладывают сверху ради конкретной выгоды из соответствующего столбца.
...
- **Дерево «Контента»** (`05-data-format.md` §3) — это в точности пара `parent` + `sibling` из §3.6, хранимая как **массив структур (AoS)**, а не как параллельные векторы (SoA). Причина — конкурентная запись: узел добавляется одной операцией, а не согласованной записью в несколько массивов сразу (`05-data-format.md` §2). Вся теория этой главы применима без изменений: SoA/AoS — вопрос раскладки в памяти, а не структуры.
- **Инвариант `parent <= id`, `sibling <= id`** (`05-data-format.md` §3) даёт однопроходные propagation (§5.3), бесплатную ацикличность (§13) и восстановление дерева последовательным сканированием «Контента» при потере TOC.
...
- **TOC** (`06-toc-format.md`), в отличие от «Контента», строится позже — одним процессом на финализации, когда дерево уже неизменно, то есть ровно в условиях Hsu (§3.7). Именно поэтому он хранится как SoA, а не AoS: тот же материал (`type`/`role`/`parent`/`sibling`), другая раскладка, плюс DFS-переупорядочение (§8) с хранением обеих таблиц соответствия (§7.2), чтобы `parent`/`sibling` оставались разрешимыми после перестановки (`06-toc-format.md` §5). В farc `sibling` в TOC хранится безусловно, даже несмотря на общее замечание §8.4 о его избыточности после DFS — он остаётся единственным документированным способом найти ключевой кадр GOP (`07-media-tree.md` §6), а отбрасывать его в пользу арифметики по позиции и предполагаемому размеру поддерева означало бы зашивать в код доменное предположение о постоянной ширине поддерева `frame`, которое переживёт не любое будущее изменение состава его детей.
...
- **Результат поиска по TOC** (`06-toc-format.md` §7, скан `value_or_offset` с фильтром по `role`) — это множество узлов, а не пара. Вопрос «в пределах одного канала/потока/версии параметров лежит выборка?» — это LCA множества (§10.6): один обратный проход по `parent`, без таблиц прыжков и без попарных вычислений.
