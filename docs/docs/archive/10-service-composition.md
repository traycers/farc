...
| Предоставлять WebSocket, отправлять нужным клиентам данные | `TocPushServer` — **новое**, §4.4 | этот документ, ADR-015 |
...
**Проблема:** ADR-015 решает *что* делать (push TOC по WebSocket подписавшимся сервисам), но не называет компонент, который это реализует, и не говорит, как соотносится с уже названным `EventStream` (SSE, `02-storage.md` §4.1.4).

**TocPushServer** (уровень Archive) — отдельный от `EventStream` компонент:
- Принимает WebSocket-подключения, ведёт подписки клиентов на конкретные каналы (клиент подписывается на интересующие его номера каналов, а не на всё хранилище).
- Подписан на `fblock.write.completed` каждого `StorageUnit` через его `NotificationBus`.
- При получении события читает TOC завершённого фконтейнера через `Reader` соответствующего `StorageUnit`.
- Определяет, каким подписчикам этот TOC релевантен (по каналам, присутствующим во фконтейнере — маска канала уже посчитана в момент финализации, ADR-014), и отправляет TOC целиком каждому подходящему подписчику.
- **Не буферизует пропущенное** при разрыве соединения (открытый вопрос ADR-015 закрыт так: буферизация не делается). Подписчик, пропустивший push, восстанавливается через fallback-чтение по `(канал, t1, t2)` — ADR-016, тот же путь, что и для впервые подключившегося подписчика.

Держать `TocPushServer` отдельно от `EventStream` — прямое следствие решения ADR-015 «отдельный канал уведомлений» (сравнение вариантов, п. 3): разный объём данных на сообщение (компактное UUID-событие у `EventStream` против полного TOC у `TocPushServer`), разная семантика повторной отправки (SSE `Last-Event-ID` навёрстывает пропущенное, WebSocket-push — нет).
...
graph TB
    ...
    VideoGW -.подписка, ADR-015.-> TocPushServer
    ...
    TocPushServer -.события.-> SU1
    TocPushServer -.события.-> SU2
    TocPushServer -->|чтение TOC| StorageRegistry
    MetricsEndpoint -.счётчики.-> SU1
    MetricsEndpoint -.счётчики.-> SU2
    EventStream -.события.-> SU1
    EventStream -.события.-> SU2
```
...
sequenceDiagram
    ...
    participant TP as TocPushServer
    participant VG as Video Gateway (WS)

    SU->>TP: event fblock.write.completed (UUID)
    TP->>SU: Reader: дай TOC фконтейнера UUID
    SU-->>TP: TOC
    TP->>TP: найти подписчиков по каналам TOC
    TP-->>VG: TOC целиком (WS-сообщение)
```
...
- **Параметры WebSocket-сервера** (`TocPushServer`): адрес, лимиты подключений — отдельно от HTTP-параметров, так как это разные серверы (могут слушать разные порты).
...
- Формат сообщения WebSocket-подписки (какими каналами/фильтрами клиент задаёт интерес в `TocPushServer`) — предмет отдельной спецификации API.
