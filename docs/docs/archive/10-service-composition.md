...
- **Фчанк** — часть фблока, единица записи с верификацией (write-verify).
...
**IngestManager** — владеет набором `ChannelIngest`, создаёт по одному на каждый канал из конфигурации, запускает и останавливает их вместе с процессом.
...
- По результату от `StorageEngine`: при успехе — обновляет снимок индексов через `IndexManager` (`N = ready`), обновляет `Catalog` на SSD, публикует `fblock.write.completed`, возвращает буфер в пул; при битом фчанке — просит `IndexManager` перевести `N` в `bad` и повторяет попытку на другом индексе (ADR-005 — запись не прерывается чтением).
- Не дожидается закрытия фконтейнера потребителем: пишет фчанки по достижении их размера или по таймауту, что раньше (ADR-017).
...
**Взаимодействие:**
- Принимает от `Recorder` команду «записать фблок на индекс N»: разбивает буфер на фчанки, пишет каждый через `IoBackend`, сразу перечитывает и сравнивает; при расхождении сообщает `Recorder`'у «фчанк битый».
- Принимает от `Reader` команду «прочитать диапазон из фблока N»: разбивает на порции меньше фчанка (ADR-005) и читает через `IoBackend`.
...
- Не знает о структуре фблока кроме разбиения на фчанки/порции — все смещения и размеры задаются вызывающим.
...
| `fblock.write.completed` | `Recorder` | После успешной записи и верификации всех фчанков |
