...
`type` (`05-data-format.md` §3) — закрытый набор физических представлений `value`, общий для всего формата, не специфичный для домена. В медиа-дереве встречаются: `void` (нет `value` — контейнер/тег ветки), `uint32`, `uint64`, `enum8`, `bytes`.
...
| `frame_kind_video`  | `enum8`  | `frame` (только video) | 0..1              | `I` (ключевой кадр) или `P` |
| `frame_codec_audio` | `enum8`  | `frame` (только audio) | 0..1              | код кодека (`pcm`/`aac`/`g711`) |

Несколько ролей намеренно делят один `type` (`params_version`/`frame_time` — обе `uint64`; `frame_kind_video`/`frame_codec_audio` — обе `enum8`) — это не проблема, а расчётный эффект разделения полей: `type` описывает только физическую раскладку `value`, семантику несёт `role`.
