...
| `codec` (video)       | `uint8`     | `data` (video)  | 1                    | `h264` = 1, `h265` = 2; 0 = `CodecUninitialized` (не пишется на диск, только Go zero-value) |
...
| `width`               | `uint32`    | `data` (video)  | 0..1                 | ширина кадра, разобрана из SPS ingest'ом; 0/отсутствует, если не удалось разобрать (`uint32`-контейнер по той же причине, что `channel`/`stream`, §3, — в формате нет `uint16`) |
| `height`              | `uint32`    | `data` (video)  | 0..1                 | высота кадра, тот же источник и оговорка, что у `width`               |
...
| `codec` (audio)       | `uint8`     | `data` (audio)  | 1                    | `pcm` = 1, `aac` = 2, `g711a` (PCMA) = 3, `g711u` (PCMU) = 4; 0 = `CodecUninitialized` (не пишется на диск, только Go zero-value) |
...
|   33 | `width`               | `…/video/configs/config/data/width`                                     |
|   34 | `height`              | `…/video/configs/config/data/height`                                    |

Коды 33/34 (`width`/`height`) появились позже остальных — добавлены в конец таблицы, не переиспользуя и не сдвигая существующие коды (append-only, §3 выше); они video-only (нет audio-аналога, поэтому не входят в восемь парных ролей ниже).
...
Практическое следствие для TOC (`06-toc-format.md` §7): запросу, которому нужны кадры обеих веток разом (например, «кадры около времени T независимо от video/audio»), приходится фильтровать по **двум** кодам роли `frame_time` — video (код 19) и audio (код 32) — вместо одного, хотя имя роли у обоих одно и то же. Это не ограничение формата: фильтр по столбцу `role` при скане `value_or_offset` (`06-toc-format.md` §4) одинаково легко принимает один код или несколько — просто раньше конкретный пример укладывался в одну роль, а теперь — в две (различаемые кодом, не именем).
...
| `width`/`height`                    | не из SDP напрямую — ingest разбирает `param_sps` (`github.com/bluenviron/mediacommon`'s `h264.SPS`/`h265.SPS`, `Width()`/`Height()`) | опциональны: узел просто не пишется, если SPS не удалось разобрать; ingest использует их (наравне с `codec`, с откатом на побайтовое сравнение `param_sps`/`param_pps`, если разрешение недоступно) для решения «изменились ли параметры потока» на границе переподключения RTSP-сессии (`11-service-composition.md` §5.1.1), а не только для отображения |
...
- ~~Схема дочерних узлов `data`, audio-ветка~~ — решено реальным SDP (AAC/`mpeg4-generic` и G.711 A-law/`PCMA`, §3.3), коды ролей зафиксированы (§3.1: `codec` (audio)=24, `sample_rate`=26, `channel_count`=27, `param_audio_config`=28). G.711 μ-law (`PCMU`) в захвате не встретился, но заведён как `g711u` = 4 по аналогии с `PCMA` (RFC 3551, тот же кодек, другой закон компандирования) — при первом реальном SDP с `PCMU` сверить, что предположение верно.
- ~~Значения `codec` (video/audio) начинались с 0, что делало Go zero-value неотличимым от реального кодека~~ — решено (`.scratch/codec-enum-values/issues/01-codecs-start-at-one.md`, 2026-08-19): оба перечисления сдвинуты, чтобы начинаться с 1 (`h264`=1/`h265`=2; `pcm`=1/`aac`=2/`g711a`=3/`g711u`=4), 0 зарезервирован как общий `CodecUninitialized` — значение, которое никогда не пишется на диск осознанно, только Go zero-value неустановленного поля. Ломающее изменение формата, принято без миграции/version-aware чтения (проект в активной разработке). Как и коды `Role` (§3 выше), значения `codec` теперь тоже append-only — однажды присвоенное значение никогда не перенумеровывается и не переиспользуется.
