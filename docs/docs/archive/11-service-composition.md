...
**IngestManager** — владеет набором `ChannelIngest`, создаёт по одному на каждый канал из конфигурации, запускает и останавливает их вместе с процессом. Обрабатывает команду смены политики захвата канала (`10-write-policy.md` §6), находя нужный `ChannelIngest` и заменяя его текущий `CapturePolicy`.
...
- На старте и при каждой смене параметров потока вызывает у Заполнителя «добавить параметры потока» (`09-fcontainer-write-path.md` §3) — эти вызовы не проходят через `CapturePolicy`, так как это метаданные потока, а не кадры (`10-write-policy.md` §3).
- Депакетизирует RTP в кадры и передаёт каждый кадр своему `CapturePolicy` (`10-write-policy.md`, один экземпляр на канал, текущая стратегия — постоянная запись/по событию/по расписанию); `CapturePolicy` решает, отправлять ли кадр Заполнителю через «добавить кадры потока» (`09-fcontainer-write-path.md` §4) немедленно, отложить в очередь, или то и другое.
...
sequenceDiagram
    ...
    CI->>CP: кадр
    Note over CP: решение по текущей стратегии<br/>(`10-write-policy.md`)
