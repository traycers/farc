...
- На старте и при каждой смене параметров потока вызывает у Заполнителя «добавить параметры потока» (`09-fcontainer-write-path.md` §3) — эти вызовы не проходят через `CapturePolicy`, так как это метаданные потока, а не кадры (`10-capture-policy.md` §3).
- Депакетизирует RTP в кадры и передаёт каждый кадр своему `CapturePolicy` (`10-capture-policy.md`, один экземпляр на канал, текущая стратегия — постоянная запись/по событию/по расписанию); `CapturePolicy` решает, отправлять ли кадр Заполнителю через «добавить кадры потока» (`09-fcontainer-write-path.md` §4) немедленно, отложить в очередь, или то и другое.
