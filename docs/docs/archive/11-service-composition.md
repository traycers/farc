...
**IngestManager** — владеет набором `ChannelIngest`, создаёт по одному на каждый канал из конфигурации, запускает и останавливает их вместе с процессом. Обрабатывает команду смены политики записи канала (`10-write-policy.md` §6), находя нужный `ChannelIngest` и заменяя его текущий `WritePolicy`.
...
- На старте и при каждой смене параметров потока вызывает у Заполнителя «добавить параметры потока» (`09-fcontainer-write-path.md` §3) — эти вызовы не проходят через `WritePolicy`, так как это метаданные потока, а не кадры (`10-write-policy.md` §3).
- Депакетизирует RTP в кадры и передаёт каждый кадр своему `WritePolicy` (`10-write-policy.md`, один экземпляр на канал, текущая стратегия — постоянная запись/по событию/по расписанию); `WritePolicy` решает, отправлять ли кадр Заполнителю через «добавить кадры потока» (`09-fcontainer-write-path.md` §4) немедленно, отложить в очередь, или то и другое.
- Управляет циклом буфера Recorder'а по команде `WritePolicy`: получить буфер (переход `idle → recording`) → заполнить через Заполнитель → вернуть с `begin`/`end`/списком каналов (переход `recording → idle`, `00-requirements.md` §4.8).
- Реагирует на статус очереди, который возвращает Recorder (норма/предупреждение/перегрузка/нет буферов, `00-requirements.md` §4.7): при перегрузке сам решает, какие данные не записывать (например, снижает GOP или дожидается ключевого кадра) — farc не отбрасывает данные сам, но кто-то выше должен, и это ChannelIngest. Независимый механизм от фильтрации `WritePolicy` — соотношение между ними открытый вопрос (`10-write-policy.md` §8).
...
sequenceDiagram
    ...
    CI->>WP: кадр
    Note over WP: решение по текущей стратегии<br/>(`10-write-policy.md`)
    WP->>CI: открыть/закрыть сегмент
    CI->>R: запросить буфер
    R-->>CI: буфер + UUID + статус очереди
    WP->>Fill: добавить кадры потока
