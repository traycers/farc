...
- На старте, при каждом переподключении (см. ниже), и (для видео) при каждом обнаруженном в потоке SPS/PPS/VPS NAL сверяет заново согласованные параметры с уже закэшированными и передаёт своему `CapturePolicy` (`10-capture-policy.md`, один экземпляр на канал) только если параметры действительно изменились — для видео по кодеку и разрешению (с откатом на побайтовое сравнение SPS/PPS, если разрешение не удалось разобрать), для аудио по кодеку, частоте дискретизации и числу каналов. Отслеживание in-band NAL для видео остаётся постоянным (а не только на границах setup/reconnect): часть камер вообще не кладёт SPS/PPS в SDP, полагаясь только на in-band пересылку перед каждым ключевым кадром — но именно семантическое сравнение, а не сам факт пересылки, решает, нужен ли новый `config`-узел, так что повторная пересылка идентичных байт (многие камеры делают это перед каждым ключевым кадром) больше не открывает новый узел на каждом кадре, как раньше. Именно `CapturePolicy` вызывает у Заполнителя «добавить параметры потока» (`09-fcontainer-write-path.md` §3) и хранит возвращаемый `id` для последующих кадров (`10-capture-policy.md` §3); момент времени нового `config`-узла — это момент первого кадра, реально записанного с этими параметрами, а не момент согласования по RTSP.
...
- Буфер Recorder'а (Заполнитель) **разделяется между всеми каналами одного хранилища**, а не выдаётся по одному на канал (`internal/ingest.StorageSegment`, multi-channel-fcontainer, см. `10-capture-policy.md` §2's примечание): переход `CapturePolicy` `idle → recording` присоединяет канал к тому общему Заполнителю, который сейчас открыт для его хранилища (открывая новый, если ни один ещё не открыт), а переход `recording → idle` лишь прекращает добавление кадров ЭТОГО канала — сам общий Заполнитель продолжает жить, пока пишут другие каналы, и закрывается (возвращается Recorder'у с `begin`/`end`/списком каналов, `00-requirements.md` §4.8) только по заполненности (ADR-017), независимо от переходов какого-либо одного канала.
...
- Переподключается при разрыве RTSP-сессии — бесконечно, с экспоненциальным backoff (капнутым, по образцу `internal/msmd`'s внешнего reconnect'а), пока канал не будет явно удалён; переподключение не влияет на уже переданные Recorder'у данные, и пробел на время разрыва виден лишь как обычный разрыв во времени между кадрами (сегмент не закрывается специально). Каждая смена статуса подключения публикуется в общий событийный поток (`channel.rtsp.connected`/`channel.rtsp.disconnected`), а текущий статус доступен через `GET /channels`.
...
- Не хранит историю — все метрики читаются по требованию; агрегацией и долговременным хранением занимается внешняя система (Prometheus/VictoriaMetrics) — по умолчанию сам стек (`docker-compose.yaml`) поднимает такую систему (Prometheus + Loki + Promtail + Grafana, `.scratch/observability/spec.md`), но это не меняет границу компонента: `MetricsEndpoint` так же работает с любым внешним скрейпером.
- Категории метрик: счётчики состояний фблоков (`uninitialized`/`ready`/`bad`/`protected`/`writable`), глубина и статус очереди записи, счётчики успешных записей и провалов верификации, активные чтения, состояние Storage (`stopped`/`running`/`job`), занятость регистра каналов. Точный список имён метрик — `02-storage.md` §8. `hls_server`/`msm_server` теперь тоже отдают `/metrics` (свободные `go_*`/`process_*` runtime-метрики через `client_golang` плюс по одному доменному гейджу каждый).
...
`write_mode` (`fill_until_full` / `cyclic`) — параметр Storage в JSON-параметрах пролога (ADR-009, `03-storage-format.md` §5.2), рядом с `retention.days`. При `write_mode = fill_until_full` `IndexManager` не выполняет приоритет 2 (`ready` с истёкшим сроком хранения) вообще — после исчерпания `uninitialized` сразу возвращает `NO_SPACE` и алерт, независимо от истёкшего срока хранения. Алгоритм `select_next_index`, учитывающий `write_mode`, — `04-storage-operations.md` §6.2.
...
> **Устарело в деталях (multi-channel-fcontainer + ADR-017, 2026-08).** Диаграмма ниже всё ещё верно показывает общий состав участников и порядок событий записи, но в двух отношениях больше не буквальна: (1) «Заполнитель» и «запросить/вернуть буфер» относятся не к одному каналу, а к общему сегменту хранилища — «открыть/закрыть сегмент» этого канала лишь присоединяет/отсоединяет его от того общего Заполнителя, который сейчас открыт (см. примечание в §5.1.1 выше); (2) физический индекс N назначается не «асинхронно, по порядку FIFO» после возврата уже целиком заполненного буфера, а сразу при получении буфера (если пул свободен) — запись на диск идёт по частям по мере заполнения (ADR-017), а не одним проходом по закрытии. Актуальный алгоритм — `internal/storage/segment.go`/`pool.go`.
