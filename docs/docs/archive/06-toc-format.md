...
|---|------|-----|----------|
| 0 | `node_count` | uint32 | Число узлов дерева = длина каждого столбца (`n`). |
| 4 | `reserved` | 60 байт | Зарезервировано, должно быть 0. |
...
```
[заголовок 64B][type[]…pad][role[]…pad][parent[]…pad][sibling[]…pad][subtree_size[]…pad][value_or_offset[]…pad][size[]]
```
...
Цена выравнивания — не более `TOC_ALIGN - 1` (63 байта) паддинга на стык между столбцами, то есть не более `6 × 63 = 378` байт на всю секцию TOC независимо от `n` (стыков между 7 столбцами — 6; сам заголовок уже кратен 64 и стыка не требует). При реалистичных `n` (миллионы узлов, §4 — 31 байт/узел без паддинга) это меньше одной десятитысячной от размера секции — постоянные накладные расходы, не растущие с `n`.
