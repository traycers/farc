...
- **Сегмент записи** — непрерывный отрезок времени, в течение которого кадры канала отправляются в один фконтейнер; соответствует одному циклу «стать активным писателем в буфер хранилища → заполнять → перестать быть активным» у `ChannelIngest` (`11-service-composition.md` §5.1.1). Сам буфер (Заполнитель) общий для всех каналов одного хранилища, одновременно в него пишущих (ADR-014, §4.1 ниже) — «один сегмент» с точки зрения канала не означает «один фконтейнер только с его данными».
...
- При переходе `idle → recording` становится активным писателем в буфер хранилища, к которому принадлежит канал (§4.1); при переходе `recording → idle` — перестаёт им быть. Ни то, ни другое не означает напрямую «фконтейнер записан на диск» — см. §4.1 про то, кто и когда на самом деле решает сбросить буфер Recorder'у.
...
### 4.1. Общий буфер нескольких каналов одного хранилища

Буфер (Заполнитель), в который CapturePolicy отправляет кадры, принадлежит не каналу, а хранилищу: все каналы, пишущие в одно хранилище, по умолчанию делят один и тот же буфер одновременно — именно так ADR-014 описывает целевой режим эксплуатации («один фконтейнер может содержать данные нескольких каналов одновременно, вплоть до постоянной записи всех каналов сразу»). Ничего в разделах 5-6 (что и когда захватывает конкретный канал — continuous/event, предзапись/постзапись, смена политики) от этого не меняется; меняется только то, что переход `recording → idle` одного канала сам по себе не может решить сбросить буфер на диск — другие каналы того же хранилища могут ещё активно писать в него, и обрыв их данных недопустим.

Поэтому решение «сбросить буфер Recorder'у прямо сейчас» принимает не сам канал, а координатор уровня хранилища, по одному из двух независимых триггеров:
- **по размеру** — сразу после очередной дозаписи кадров, если накопленный буфер достиг `~min_container_share × fblock_size` (`fblock/params.go`'s `MinContainerShare`, уже существующий параметр геометрии, здесь переиспользуемый как рабочая цель заполнения, а не только как проверяемый при инициализации инвариант);
- **по последнему отключившемуся каналу** — когда канал переходит `recording → idle`, а больше ни один канал этого хранилища не пишет в текущий буфер, он сбрасывается немедленно, не дожидаясь порога по размеру (сохраняет привычную отзывчивость «остановил запись → почти сразу записано на диск» для типичного случая, когда у хранилища один активный канал).

Оба триггера сбрасывают один и тот же буфер целиком, независимо от того, чьи именно кадры в него попали — итоговый `WriteFcontainer` покрывает объединение каналов-участников, а не только тот канал, чей переход состояния оказался триггером.
