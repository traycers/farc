...
Остаётся открытым, что происходит со сменой параметров, пока сегмент не открыт (`idle`, буфер не запрошен — тогда нет ни фконтейнера, ни Заполнителя, которому можно позвонить) — раздел 8.
...
- Смена параметров потока, пока сегмент не открыт (`idle`) — нет ни фконтейнера, ни Заполнителя, которому можно передать «добавить параметры потока». Вероятно, `CapturePolicy` должна сама кэшировать последние известные параметры потока (независимо от состояния сегмента) и при открытии нового сегмента (`idle → recording`) первым делом воспроизвести Заполнителю нужный `config` (или несколько — если за время в очереди/предзаписи параметры менялись не один раз, и в неё попали кадры разных `config`, каждый должен быть добавлен в новое дерево по отдельности, в порядке возникновения, прежде чем будут добавлены сами кадры) — до первого вызова «добавить кадры потока». Не решено.
