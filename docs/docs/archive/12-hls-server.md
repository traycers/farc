...
**Ответственность:** поддерживает WebSocket-подключение к `EventPushServer` каждого сконфигурированного хранилища farcd, подписывается на TOC по всем каналам, которые обслуживает `hls_server`.
...
Переменные окружения (`HLS_SERVER_HTTP_IP`/`HLS_SERVER_HTTP_PORT`, `HLS_SERVER_TARGET_SEGMENT_DURATION`, `HLS_SERVER_CACHE_DIR`, `HLS_SERVER_CACHE_QUOTA_BYTES`):
...
- Список хранилищ farcd, с которыми работает `hls_server`: адрес `HttpApiServer`, адрес `EventPushServer`.
- Список каналов, которые обслуживает `hls_server`, с привязкой канала к хранилищу (та же связка `канал → хранилище`, что задана в конфигурации самого farcd, `11-service-composition.md` §9 — источник истины для этой связки между двумя процессами не определён, см. §8).
...
- **Источник истины для связки `канал → хранилище`.** Сейчас она задаётся конфигурацией `farcd` (`11-service-composition.md` §9) и должна быть продублирована в конфигурации `hls_server` (§7) — схема синхронизации (ручная, через API `farcd`, и т.д.) не определена.
