...
GET    /storages/{id}/fblocks                     -> [{index,state,uuid?,begin?,end?,protected,channels?}] (Phase 22, whole-storage, no channel filter)
...
| `web/src/api/events.ts` | Phase 18: `subscribeJournal`, an auto-reconnecting WS client for the Journal feed. Phase 22: `subscribeLive` (channel-scoped, also dispatches `"live"` frames) |
...
| `web/src/pages/FblockLivePage.tsx`, `web/src/pages/FblockStatusPage.tsx` | Phase 22: live-recording-segment tree / finalized-fblock tree, both rendered by `FblockTree.tsx` |
...
- `internal/api/fblocks.go` (Phase 22) — `handleListFblocks`, the whole-storage fblock table endpoint.
