...
func setRequiredEnv(t *testing.T) {
	t.Helper()
	t.Setenv("HLSD_HTTP_PORT", "8090")
	t.Setenv("HLSD_METRICS_PORT", "9091")
	t.Setenv("HLSD_FARC_HTTP", "http://10.0.0.1:8080")
	t.Setenv("HLSD_FARC_WS", "ws://10.0.0.1:8081")
	t.Setenv("HLSD_TARGET_SEGMENT_DURATION", "6s")
	t.Setenv("HLSD_CACHE_DIR", "/var/cache/hlsd")
	t.Setenv("HLSD_CACHE_QUOTA_BYTES", "10737418240")
}

// docExample is the JSON-backed part of hlsd's config -- http, the
...
func TestLoad_DocExample(t *testing.T) {
	...
	if cfg.CacheDir != "/var/cache/hlsd" {
...
func TestLoad_CustomEnvAddr(t *testing.T) {
	t.Setenv("HLSD_HTTP_IP", "127.0.0.1")
	t.Setenv("HLSD_HTTP_PORT", "18090")
	t.Setenv("HLSD_METRICS_PORT", "19091")
	t.Setenv("HLSD_FARC_HTTP", "http://h")
	t.Setenv("HLSD_FARC_WS", "ws://h")
	t.Setenv("HLSD_TARGET_SEGMENT_DURATION", "2s")
	t.Setenv("HLSD_CACHE_DIR", "/tmp/cache")
...
func TestLoad_MissingHTTPPortEnvRejected(t *testing.T) {
	...
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_HTTP_PORT, got nil")
	}
}
...
func TestLoad_MissingMetricsPortEnvRejected(t *testing.T) {
	...
	const doc = `{"channels": []}`
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_METRICS_PORT, got nil")
	}
}

func TestLoad_InvalidPortEnvRejected(t *testing.T) {
	setRequiredEnv(t)
	t.Setenv("HLSD_HTTP_PORT", "not-a-number")

	const doc = `{"channels": []}`
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for non-integer HLSD_HTTP_PORT, got nil")
	}
}
...
func TestLoad_MissingFarcHTTPEnvRejected(t *testing.T) {
	...
	const doc = `{"channels": []}`
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_FARC_HTTP, got nil")
	}
}
...
func TestLoad_MissingFarcWSEnvRejected(t *testing.T) {
	...
	const doc = `{"channels": []}`
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_FARC_WS, got nil")
	}
}
...
func TestLoad_MissingTargetSegmentDurationEnvRejected(t *testing.T) {
	...
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_TARGET_SEGMENT_DURATION, got nil")
	}
}
...
func TestLoad_MissingCacheDirEnvRejected(t *testing.T) {
	...
	const doc = `{"channels": [{"id":1,"storage":"disk0"}]}`
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_CACHE_DIR, got nil")
	}
}
...
// cache_dir holds hlsd's persistent TOC cache (a dedicated subpath
...
func TestLoad_MissingCacheDirEnvRejected_S3Backend(t *testing.T) {
	...
	t.Setenv("HLSD_FARC_WS", "ws://h")
	t.Setenv("HLSD_TARGET_SEGMENT_DURATION", "6s")
	t.Setenv("HLSD_CACHE_BACKEND", "s3")
	t.Setenv("HLSD_S3_ENDPOINT", "http://minio:9000")
	t.Setenv("HLSD_S3_BUCKET", "hls")

	const doc = `{"channels": [{"id":1,"storage":"disk0"}]}`
	if _, err := Load(writeConfig(t, doc)); err == nil {
		t.Fatalf("Load: want error for missing HLSD_CACHE_DIR even with cache_backend=s3, got nil")
	}
}
...
func TestLoad_InvalidDurationEnvRejected(t *testing.T) {
	...
	t.Setenv("HLSD_FARC_HTTP", "http://h")
	t.Setenv("HLSD_FARC_WS", "ws://h")
	t.Setenv("HLSD_TARGET_SEGMENT_DURATION", "6 seconds")
	t.Setenv("HLSD_CACHE_DIR", "/tmp/cache")
